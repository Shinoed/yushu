...
// Helper to get absolute offsetTop of an element relative to the document body
...
function scrollToTop() {
  if (scriptureContent) {
    // Get absolute position relative to body to avoid offsetParent container limitations
    const targetY = getAbsoluteOffsetTop(scriptureContent);
    
    // Scroll window smoothly to the exact Y coordinate of scriptureContent
    window.scrollTo({
      top: targetY - 10, // 10px gap from top for better aesthetics
      behavior: 'smooth'
    });
